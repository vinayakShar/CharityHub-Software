...
app.post('/pay', (req, res) => {
  ...
  const create_payment_json = {
    ...
    "redirect_urls": {
        "return_url": "/success", // Change to 'http://localhost:8080/success' for local
        "cancel_url": "/cancel"  // Change to 'http://localhost:8080/cancel' for local
    },
    "transactions": [{
        "item_list": {
            "items": [{
...
app.get('/success', (req, res) => {
  ...
  paypal.payment.execute(paymentId, execute_payment_json, function (error, payment) {
      if (error) {
          ...
          res.redirect('/'); // Change to 'http://localhost:8080' for local
      }
  });
  });
